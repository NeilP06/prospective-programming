...
function CheckCode(props) {
    ...
    const handleSubmit = async(e) => {
        ...
        try {
            ...
            // retreives GPT response as a variable:
            const codeData = JSON.parse(result.data.choices[0].message.content);
            // sets `correct` & `hint` variables based upon deciphered GPT data:
...
function Feedback(props) {
    ...
    if (props.correct === false) {
        return (
            <div className="px-7 py-2 mt-5 flex justify-center w-[500px] border-2 border-amber-500 rounded-md bg-amber-700">
